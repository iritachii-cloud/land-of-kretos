...
export class SelectionScene extends Scene {
    ...
    renderModeSelection(ctx) {
        ...
        ctx.fillStyle = '#aaa';
        ctx.textAlign = 'center';
        ctx.fillText('↑↓ to choose · ENTER to confirm · ESC to back', width/2, height - 50);
    }
    ...
    renderStageSelection(ctx) {
        ...
        ctx.fillStyle = '#aaa';
        ctx.textAlign = 'center';
        ctx.fillText('←→↑↓ to select · ENTER to confirm · ESC to back', width/2, height - 50);
    }
    ...
    renderHeroSelection(ctx) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(`PLAYER ${this.activePlayer} — BROWSE [←→↑↓] · CONFIRM [ENTER]`, width/2, height - 95);
        ctx.font = '14px monospace';
        ctx.fillStyle = '#aaa';
        ctx.fillText('PAGE [  ] · UNDO [ESC]', width/2, height - 70);
    }
    ...
    handleInput(e) {
        ...
        if (this.modal.visible) {
            if (e.code === 'ArrowLeft') { this.modal.selected = 0; this.game.soundManager?.play('select'); }
            else if (e.code === 'ArrowRight') { this.modal.selected = 1; this.game.soundManager?.play('select'); }
    ...
    handleModeInput(e) {
        ...
        if (e.code === 'ArrowUp') { idx = (idx - 1 + modes.length) % modes.length; this.game.soundManager?.play('select'); }
        if (e.code === 'ArrowDown') { idx = (idx + 1) % modes.length; this.game.soundManager?.play('select'); }
        this.mode = modes[idx];
        if (e.code === 'Enter') { this.game.soundManager?.play('confirm'); this.state = 'stage'; }
        if (e.code === 'Escape') { this.game.soundManager?.play('back'); this.game.sceneManager.switchTo('menu'); }
    }
    ...
    handleStageInput(e) {
        ...
        const cols = 3;
        if (e.code === 'ArrowLeft') { idx = Math.max(0, idx-1); this.game.soundManager?.play('select'); }
        if (e.code === 'ArrowRight') { idx = Math.min(stages.length-1, idx+1); this.game.soundManager?.play('select'); }
        if (e.code === 'ArrowUp') { idx = Math.max(0, idx-cols); this.game.soundManager?.play('select'); }
        if (e.code === 'ArrowDown') { idx = Math.min(stages.length-1, idx+cols); this.game.soundManager?.play('select'); }
        ...
        if (e.code === 'Escape') { this.game.soundManager?.play('back'); this.state = 'mode'; }
    }
    
    handleHeroInput(e) {
        const moved = e.code === 'BracketLeft' || e.code === 'BracketRight' || e.code.startsWith('Arrow');
        if (moved) this.game.soundManager?.play('select');
        
        if (e.code === 'BracketLeft') this.gridSelector.prevPage();
        if (e.code === 'BracketRight') this.gridSelector.nextPage();
        if (e.code === 'ArrowUp') this.gridSelector.moveCursor(0, -1);
        if (e.code === 'ArrowDown') this.gridSelector.moveCursor(0, 1);
        if (e.code === 'ArrowLeft') this.gridSelector.moveCursor(-1, 0);
        if (e.code === 'ArrowRight') this.gridSelector.moveCursor(1, 0);
