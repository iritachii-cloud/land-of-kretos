...
export class SelectionScene extends Scene {
    ...
    renderModeSelection(ctx) {
        ...
        ctx.fillStyle = '#aaa';
        ctx.textAlign = 'center';
        ctx.fillText('↑↓ to choose · ENTER / ⚡P to confirm · ESC / 👟K to back', width / 2, height - 50);
    }
    ...
    renderStageSelection(ctx) {
        ...
        ctx.fillStyle = '#aaa';
        ctx.textAlign = 'center';
        ctx.fillText('←→↑↓ to select · ENTER / ⚡P to confirm · ESC / 👟K to back', width / 2, height - 50);
    }
    ...
    renderHeroSelection(ctx) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(`PLAYER ${this.activePlayer} — BROWSE [←→↑↓] · CONFIRM [ENTER / ⚡P]`, width / 2, height - 95);
        ctx.font = '14px monospace';
        ctx.fillStyle = '#aaa';
        ctx.fillText('PAGE [  ] · UNDO [ESC / 👟K]', width / 2, height - 70);
    }
    ...
    handleModeInput(e) {
        ...
        const isBack = e.code === 'Escape' || e.code === 'KeyI';

        if (isUp) { idx = (idx - 1 + modes.length) % modes.length; this.game.soundManager?.play('select'); }
        if (isDown) { idx = (idx + 1) % modes.length; this.game.soundManager?.play('select'); }
        this.mode = modes[idx];
        if (isConfirm) {
            this.game.soundManager?.play('confirm');
            this.state = 'stage';
    ...
    handleStageInput(e) {
        ...
        if (isLeft) { idx = Math.max(0, idx - 1); this.game.soundManager?.play('select'); }
        if (isRight) { idx = Math.min(stages.length - 1, idx + 1); this.game.soundManager?.play('select'); }
        if (isUp) { idx = Math.max(0, idx - cols); this.game.soundManager?.play('select'); }
        if (isDown) { idx = Math.min(stages.length - 1, idx + cols); this.game.soundManager?.play('select'); }
    ...
    handleHeroInput(e) {
        ...
        const isConfirm = e.code === 'Enter' || e.code === 'KeyU';
        const isBack = e.code === 'Escape' || e.code === 'KeyI';
        const isPageLeft = e.code === 'BracketLeft';
        const isPageRight = e.code === 'BracketRight';

        const moved = isUp || isDown || isLeft || isRight || isPageLeft || isPageRight;
        if (moved) this.game.soundManager?.play('select');

        if (isPageLeft) this.gridSelector.prevPage();
        if (isPageRight) this.gridSelector.nextPage();
        if (isUp) this.gridSelector.moveCursor(0, -1);
        if (isDown) this.gridSelector.moveCursor(0, 1);
        if (isLeft) this.gridSelector.moveCursor(-1, 0);
        if (isRight) this.gridSelector.moveCursor(1, 0);
